...
async function processOlympicHill() {
  ...
  try {
    ...
    const projectData = {
      ...
      description:
        "Downtown LA high-rise construction project by Onni Construction. $500M budget, 30-month timeline, 7-level underground parking.",
      tags: [
        "downtown-la",
        "high-rise",
        "onni-construction",
    ...
    console.log(
      `   📦 Total chunks stored: ${stats.vector_store.total_chunks}`
    );
    ...
    for (const query of testQueries) {
      ...
      if (retrievalResult.chunks.length > 0) {
        ...
        console.log(
          `   📊 Best similarity score: ${retrievalResult.chunks[0].similarity.toFixed(
            3
          )}`
        );
        console.log(
          `   📄 Context preview: ${retrievalResult.context
            .substring(0, 150)
            .replace(/\n/g, " ")}...`
        );
      } else {
    ...
    console.log(
      '   3. Include project_id: "olympic-hill" in your chat requests'
    );
    ...
    console.log(
      `     "message": "What are the main challenges for Olympic Hill?",`
    );
